    ...
    <div id='order_details'>
        ...
        <?php if(count($order_details)!=0){?>
            ...
            <?php   foreach ($order_details as $od) {?>
                ...
                <tr class='<?=$class?> odid<?=$od->id?>' id='od_product_<?=$od->product_id?>'>
                    <?php $sub = $od->quantity * $od->price ?>
                  <td><?=$od->arabic?></td>
                  <td data-qty='<?=$od->quantity?>' class='qty'><?=$od->quantity?> </td>
                  <td class='price'><?=$od->price?></td>
                  <td class='sub'><?=$sub?></td>
                  <td> <a data-id=<?=$od->id?> class='delete table-actions-button ic-table-delete'></a></td>
                  <td> <a data-id=<?=$od->id?> class='table-actions-button ic-table-edit'></a></td>
                  <?=form_hidden('order_details_id', $od->id)?>
                </tr>
              <?php $i++;$sum += $sub;
          } 
        ?>
