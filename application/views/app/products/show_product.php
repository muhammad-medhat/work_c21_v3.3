...
<?php ######################################################?>
          <div class='col fl'>
            <select id='all_items' class='items' size=<?=count($all_items)/2?> >
              ...
              <?php //check for the assigned items before adding ?>
...
<script type="text/javascript" charset="utf-8">
  $(document).ready(function () {
    ...
    function show_item_window(product_id, item_id){
      $('#panel').show();
      var url = 'product/show_item_window' + '/' + product_id  + '/' + item_id;
      $('#panel').load('<?=site_url()?>/' + url);
    }
    
    $('#panel').on('click', '#delete_item', function(){
      /*unassign item*/
          ...
          var item_option ="<option value='" + id + "'>" + name + "</option>";
            ...
            $('#items option:selected').remove(); //removes item from the listbox
    ...
    $('#panel').on('click', '#save_item', function(){
      ...
      //alert(amount);
      $.ajax({
      url: '<?=site_url('product/assign')?>/' + product_id + '/' + item_id,// + '/' + amount,
        type: 'POST',
    ...
    function remove_item(pid, iid){
        $.ajax({
        url: '<?= site_url('product/unassign')?>/' + pid + '/' + iid,
          type: 'POST',
          complete: function (jqXHR, textStatus) { },
          success: function (data, textStatus, jqXHR) {  },
          error: function (jqXHR, textStatus, errorThrown) {  }
  ...
  function assign_item(pid, iid){
    $.ajax({
    url: '<?= site_url('product/assign_ajax')?>/' + pid + '/' + iid+ '/'+  '0.1',
      type: 'POST',
      complete: function (jqXHR, textStatus) { },
      success: function (data, textStatus, jqXHR) {
        $('.log').text(data);
